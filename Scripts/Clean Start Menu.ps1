...
foreach($dir in $userFolders)
{
	if(!$exemptFolders.Contains($dir))
	{
		...
		{
			Move-Item -Force -Path "$currentFolder\$(Get-ChildItem $currentFolder -name)" -Destination "$userStartPath\$(Get-ChildItem $currentFolder -name)"
			Remove-Item -Recurse $currentFolder
		}
	}
}
